...
class UpdateBankConnectionParams implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function __construct(array $data = null)
    {
        ...
        $this->setIfExists('force_web_form', $data ?? [], false);
    ...
    */
     ...
     * Sets banking_pin
     *
     * @param string|null $banking_pin THIS FIELD IS DEPRECATED AND WILL BE REMOVED.<br/>Please refer to the 'loginCredentials' and 'interface' fields instead. If any of those two fields is used, then the value of this field will be ignored.<br><br>Online banking PIN. Any symbols are allowed. Max length: 170. If a PIN is stored in the bank connection, then this field may remain unset. If finAPI's Web Form is not required and the field is set though then it will always be used (even if there is some other PIN stored in the bank connection). If you want the user to enter a PIN in finAPI's Web Form even when a PIN is stored, then just set the field to any value, so that the service recognizes that you wish to use the Web Form flow.
     *
     * @return self
     */
     ...
     *
     * @param bool|null $import_new_accounts Whether to import accounts that weren’t imported previously. Default is false.<br/><br/>NOTES: <br/>&bull; For best performance, you should not enable this flag unless you really expect new accounts to be available in the connection. It is recommended to let your users tell you through your application when they want the service to look for new accounts.<br/>&bull; If you have imported an interface using a limited set of 'accountTypes', you would import all other accounts when setting this flag to true. To avoid importing account types that you are not interested in, make sure to keep this flag unset (or set to false).<br/>&bull; If there is an existing consent and it doesn’t support the import of new accounts (see 'BankConnectionInterface.aisConsent.supportsImportNewAccounts'), then setting 'importNewAccounts=true' will have no effect.
     *
     * @return self
     */
    ...
    public function setForceWebForm($force_web_form)
    {
        ...
        if (is_null($force_web_form)) {
            throw new \InvalidArgumentException('non-nullable force_web_form cannot be null');
        }
